...
def main():
    ...
    allRobots = []
    sim = rvo2.PyRVOSimulator(1/2., 100, 50, 2, 2, 10, 1.0, (0, 0))
    ...
    for i in range(0, mainRobotNumber):
        ...
        mainRobot = mainRobot + [sim.addAgent((initPosMainRobot[i][0], initPosMainRobot[i][1]))]
    ...
    for e in range(num_episodes):
        ...
        for i in range(0, mainRobotNumber):
            ...
            sim.setAgentPosition(mainRobot[i], (initPosMainRobot[i][0], initPosMainRobot[i][1]))
        for i in range(0, obsNumber):
            [posObstRobot_msg[i].pose.position.x, posObstRobot_msg[i].pose.position.y] = initPosObstRobot[i]
            posObstRobot_msg[i].pose.position.z = 0
        ...
        while not done:
            ...
            for i in range(0, mainRobotNumber):
                ...
                sim.setAgentPrefVelocity(mainRobot[i], ((goalPos[i][0] - initPosMainRobot[i][0]) / math.sqrt((goalPos[i][0] - initPosMainRobot[i][0])**2 + (goalPos[i][1] - initPosMainRobot[i][1])**2 ), (goalPos[i][1] - initPosMainRobot[i][1]) / math.sqrt((goalPos[i][0] - initPosMainRobot[i][0])**2 + (goalPos[i][1] - initPosMainRobot[i][1])**2)))
            for i in range(0, obsNumber):
                model_coordinates = rospy.ServiceProxy('gazebo/get_model_state', GetModelState)
            ...
            for curRobNo in range(0, mainRobotNumber):
                ...
                # rospy.logwarn(sim.getAgentPosition(mainRobot[curRobNo]))
                [linearX, angularZ] = takeAction([sim.getAgentPosition(mainRobot[curRobNo])[0] - object_coordinates[curRobNo].pose.position.x, sim.getAgentPosition(mainRobot[curRobNo])[1] - object_coordinates[curRobNo].pose.position.y], yaw)
